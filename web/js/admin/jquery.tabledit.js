...
var input = '<select data-placeholder="Seleccione una opcion" class="chosen-select custom-select form-control validate tabledit-input" tabindex="5" name="' + settings.columns.editable[i][1] + '" style="display: none;" disabled>';

                                // Create options for select element.
                                $.each(jQuery.parseJSON(settings.columns.editable[i][2]), function(index, value) {
                                    if (text === value) {
                                        input += '<option value="' + value.id + '" selected>' + value.data + '</option>';
                                    } else {
                                        input += '<option value="' + value.id + '">' + value.data + '</option>';
                                    }
                                });
